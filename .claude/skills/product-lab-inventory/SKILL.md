...
Use this workflow when the owner asks to update inventory from a physical count. Claude interprets the source; the deterministic operator and Product Lab database decide what may change.
...
Read the supplied file or pasted table yourself. Do not install parsers, OCR, or AI APIs. Preserve each source name and count as written, then create a transient JSON file under the ignored `.inventory-operator/intents/` directory with:
...
The launcher/process must receive `PRODUCT_LAB_SUPABASE_URL`, `PRODUCT_LAB_SUPABASE_PUBLISHABLE_KEY`, and a short-lived `PRODUCT_LAB_OWNER_ACCESS_TOKEN`. The project key must be modern `sb_publishable_*` or a legacy JWT carrying `role: anon`; the operator rejects secret, service-role, and other `sb_*` forms locally before any request. Never request or store a password, database password, secret/service-role key, or token in a file. The operator validates the owner token with Supabase Auth and requires `app_metadata.app_role == "owner"`; the database owner check remains authoritative. See `docs/INVENTORY_OPERATOR.md` for the temporary manual handoff from Product Lab's existing browser session.
...
When the owner resolves an ambiguous or suggested row, preserve its original `raw_name` and add the explicitly selected canonical name as `match_name`. Never add `match_name` based only on Claude's guess. The final preview shows and binds both names and the selected canonical ingredient. The CLI can enforce a permitted final match, but cannot prove who supplied `match_name`; this prohibition is a Claude workflow rule.
...
Only after the owner supplies the matching code, run:

```text
npm run inventory-operator -- inventory:count-apply --preview-id <preview_id> --approval-code <code>
npm run inventory-operator -- inventory:verify --preview-id <preview_id>
```

Never edit a preview artifact and never call SQL, a generic RPC, or `apply_raw_inventory_adjustment` directly. The apply command accepts only a stored preview and exact approval code. The database applies the whole file atomically through Product Lab's existing authority.

If apply reports stale inventory or a changed preview, rerun preview and require a new approval. If the response is lost, retry the identical apply command; do not create a new occurrence or add a force flag. Never report completion until `inventory:verify` returns `status: "verified"`.
